...
void process_sell_info(vector<sellday> &sellDate, string inp){
	...
	//cout<<sellDate[1].sold_product[product_id].sell_state[state_id].totalSell <<endl;
...
int get_query_answer(vector<sellday> &sellDate, int dayname, int p_id, int pc_id, int s_id, int sr_id){
	...
	if(p_id==-1){
		if(s_id==-1){
			ret_result = sellDate[dayname].totalSell;
		}else{
			ret_result = 0;
			FOR(i,1,MAX_PRODUCT_ID){
				if(sr_id!=0){
					ret_result += sellDate[dayname].sold_product[i].sell_state[s_id].region_id[sr_id];
				}else{
					ret_result += sellDate[dayname].sold_product[i].sell_state[s_id].totalSell;
				}
			}
		}
	}else{
		if(pc_id==0){
			...
			if(s_id==-1){
				ret_result = sellDate[dayname].sold_product[p_id].category_id[pc_id].totalSell;
			}else{
...
void process_query_info(vector<sellday> &sellDate, string inp){
	...
	len++;
	while((product_id!=-1)&&(len<product_details.size())){
		if(product_details[len]==' '){
			break;
		}else{
			product_category_id = ((10*product_category_id) + (product_details[len]-'0'));
	...
	len++;
	while((state_id!=-1)&&(len<region_details.size())){
		if(region_details[len]==' '){
			break;
		}else{
			state_region_id = ((10*state_region_id) + (region_details[len]-'0'));
	...
	int result = 0;
	if(d_end!=0){
		FOR(i,d_start,d_end+1){
			result += get_query_answer(sellDate,i,product_id,product_category_id,state_id,state_region_id);
		}
	}else{
		result += get_query_answer(sellDate,d_start,product_id,product_category_id,state_id,state_region_id);
	}
	cout<<result<<endl;
	/*cout<<"query_type="<<s_or_q<<endl;
