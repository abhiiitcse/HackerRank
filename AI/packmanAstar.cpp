...
PackmanAstar::PackmanAstar(){
}

PackmanAstar::~PackmanAstar(){
}

void PackmanAstar::readInp(void){
...
void PackmanAstar::solveprob(){
  ...
  int h[boardsize[0]][boardsize[1]];
  int pr[boardsize[0]][boardsize[1]];
  FOR(i,0,boardsize[0]){
    FOR(j,0,boardsize[1]){
      g[i][j] = 10000;
      h[i][j] = fabs(foodpos.posr - i) + fabs(foodpos.posc - j);
      visited[i][j] = false;
      pr[i][j] = 10000;
    }
  }
  ...
  pqtemp.priority = h[packmanpos.posr][packmanpos.posc];
  pq.push(pqtemp);
  pr[packmanpos.posr][packmanpos.posc] = g[packmanpos.posr][packmanpos.posc] + h[packmanpos.posr][packmanpos.posc];
  ...
  while(!pq.empty() && (!foodfound)){
	...
	g[temppos.posr][temppos.posc] = g[popnode.node.posr][popnode.node.posc] + 1;
      ...
      if(pr[temppos.posr][temppos.posc] > (g[temppos.posr][temppos.posc] + h[temppos.posr][temppos.posc])){
	pqtemp.priority = g[temppos.posr][temppos.posc] + h[temppos.posr][temppos.posc];
	pr[temppos.posr][temppos.posc] = g[temppos.posr][temppos.posc] + h[temppos.posr][temppos.posc];
	pq.push(pqtemp);
